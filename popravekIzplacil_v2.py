...
def getData(workbook):
    ...
    for i in range(len(sheet3["B"])):
        if sheet3["B"][i].value == "Maturity Claim Payment (To Client)" or sheet3["B"][i].value == "Surrender Claim Payment (To Client)" or sheet3["B"][i].value == "Scholarship Payment to client":
        ...
        if sheet3["B"][i].value == "Personal Income tax (Maturity)" or sheet3["B"][i].value == "Personal Income tax (Surrender)" or sheet3["B"][i].value == "Personal Income tax (Scholarship)":
...
def generateOutput(data):
    ...
    if data["claims_dets_claim_amt"] and data["sa_postings_sa_transno"]:
        description3          = "V tabeli claim_dets, kjer je SA_POSTINGNO = {}, je treba nastaviti vrednost CLAIM_TAX = 0.00 ter popraviti vrednost v stolpcih CLAIM_AMT in CLAIM_NET_AMT:\n\nstara vrednost: {}\nnova vrednost: {}"\
...
def main():
    ...
    output = generateOutput(getData(wb))
    fh = open("popravek_izplacil_{}_{}.txt".format(output[1], date.today().strftime("%Y-%m-%d")), "w")
    fh.write(output[0])
    fh.close()
    print("Ustvarjeni podatki shranjeni v datoteko popravek_izplacil_{}_{}.txt".format(output[1], date.today().strftime("%Y-%m-%d")))
